...
public class DatagramSocketQ1_Client
{
    public static void main(String[] args)
    {
        Thread receieveThread = new Thread(new Runnable()
        {
            @Override
            public void run()
            {
        ...
        try(DatagramSocket datagramSocket = new DatagramSocket(); BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(System.in)))
        {
            ...
            while (!clientMsg.equals("stop"))
            {
                ...
                DatagramPacket datagramPacket = new DatagramPacket(clientMsg.getBytes(), clientMsg.length(), Inet4Address.getByName("127.0.0.1"), 3333);
